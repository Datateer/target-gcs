...
def build_sink(config={}):
    ...
    print(232222, config)
    return GCSSink(TargetGCS(config=config), 'my_stream', {'properties': {}}, key_properties=config)
...
def test_key_name_includes_date_format_if_date_token_used_and_date_format_provided():
    date_format = '%m %d, %Y'
    subject = build_sink({'key_naming_convention': 'file/{date}.txt', 'date_format': date_format})
    assert f'file/{datetime.today().strftime(date_format)}.txt' == subject.key_name
...
def test_key_name_includes_timestamp_if_timestamp_token_used():
    ...
    assert re.match(f'file/\d+.txt', subject.key_name)