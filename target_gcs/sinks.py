...
class GCSSink(BatchSink):
    ...
    def build_key_name(self, config):
        ...
        key_name = self.config.get("key_naming_convention", f"{self.stream_name}_{extraction_timestamp}.{self.output_format}")
        key_name = f'{self.config.get("key_prefix", "")}/{key_name}'.replace('//', '/')
        if key_name.startswith('/'):
            key_name = key_name[1:]
        date = datetime.today().strftime(self.config.get('date_format', '%Y-%m-%d'))
        key_name = key_name.format_map(defaultdict(str, stream=self.stream_name, date=date, timestamp=extraction_timestamp))
        return key_name
    ...
    def gcs_write_handle(self):
        ...
        if not self._gcs_write_handle:
            ...
            self._gcs_write_handle = open(f'gs://{self.config.get("bucket_name")}/{self.key_name}', 'wb', transport_params=dict(client=Client.from_service_account_json(credentials_path)))
